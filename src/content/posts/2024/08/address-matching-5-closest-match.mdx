...
This function is going to be called in a linq query allowing us to match and story based on the best match. Given any set of address, candidates might have multiple values, for example different flats in a building, we need to match the remaining input address fields against the set of available candidates.
