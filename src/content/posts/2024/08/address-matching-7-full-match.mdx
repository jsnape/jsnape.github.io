...
```csharp
    internal static (bool, MatchState) MatchPart(
        MatchState state, Func<Address, string?> candidateSelector, int maxTolerance)
    {
        ...
        if (candidateValues.Length == 0)
        {
            ...
            return (true, UnmatchState(state));
        }

        var matches = candidateValues
            .SelectMany(c => state.UnmatchedParts.Select(p => (c, p, MatchDistance(c, p))))
            .OrderBy(m => m.Item3);
        ...
        if (part == null)
        {
            ...
            // and should stop.
            return (candidateValues.Length == 0, UnmatchState(state));
        }
    ...
    // Helper function to clear the match state.
    internal static MatchState UnmatchState(MatchState state)
    {
        return state with
        {
...
```csharp
    private static MatchState SplitMatchedField(MatchState state)
    {
        ...
        if (candidate.Length < matched.Length)
        {
            var leftPart = matched[..(matched.Length - candidate.Length)].Trim();

            state = state with
            {
                UnmatchedParts = state.UnmatchedParts.Add(leftPart)
            };
        }
