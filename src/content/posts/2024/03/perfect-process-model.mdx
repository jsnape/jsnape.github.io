...
The Basic process model it reduced to a bare minimum. All work items are simply `Issues` with a higher level `Epic`. To differentiate different types of work you use tagging in the same way GitHub uses [labels on its issues](https://docs.github.com/en/issues/using-labels-and-milestones-to-track-work/managing-labels). Issue states are `To Do > Doing > Done` without extra process steps too.
