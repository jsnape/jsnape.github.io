---
title: "The hidden effects of queue on cycle time"
postDate: 2024-03-02
description: >-
  Queues are used to load balance work between teams but its easy to overlook decrease in efficiency they bring.
image:
...
Lets visualize this by creating an ideal delivery pipeline. Imagine we have 10 work items; all exactly the same size; so they all take exactly one day to complete at every stage. (I know this isn't the case in reality but reality only makes things worse.) The delivery plan as work flows through the process is shown below:
...
| Day   | Backlog | Specify | Build | Verify | Release | Done |
| ----- | ------- | ------- | ----- | ------ | ------- | ---- |
|       | Waiting | Doing   | Doing | Doing  | Doing   | Done |
| 1     | 10      |         |       |        |         |      |
| 2     | 9       | 1       |       |        |         |      |
| 3     | 8       | 1       | 1     |        |         |      |
| 4     | 7       | 1       | 1     | 1      |         |      |
...
The stats for this configuration are interesting - for the first time the active time has gone up because we are asking the release time to do 4 more releases. Similarly, the calendar time remains the same because we still limited by the time it takes to get the last work items released but there is far less waiting and the overall cycle time has dropped to just 7.5 days.
